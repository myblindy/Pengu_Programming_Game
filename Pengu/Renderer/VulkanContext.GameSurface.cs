...
using Pengu.Support;
using static Pengu.Renderer.VulkanContext;

namespace Pengu.Renderer
{
    internal class PenguGameSurface : GameSurface
    {
        readonly List<BaseWindow> Windows = new List<BaseWindow>();
        internal Font monospaceFont;
        readonly FontString fontStringFps;
        ...
        private void MoveWindowToTop(BaseWindow? window)
        {
            ...
            {
                monospaceFont.MoveStringToTop(window.ChromeFontString);
                monospaceFont.MoveStringToTop(window.ContentFontString);
        ...
        public PenguGameSurface(VulkanContext context) : base(context)
        {
            this.context = context;
            Modules.Add(monospaceFont = new Font(context, "pt_mono"));

            fontStringFps = monospaceFont.AllocateString(new Vector2(-1f * context.Extent.AspectRatio, -.995f), .033f);
            fontStringFps.Set(defaultBg: FontColor.Black, defaultFg: FontColor.White);

            var (u0, v0, u1, v1) = monospaceFont[' '];
        ...
        {
            fontStringFps.Set($"FPS: {FPS:0.00} Font Verts: {monospaceFont.UsedVertices} used out of {monospaceFont.MaxVertices}");
            base.UpdateLogic(elapsedTime);

            monospaceFont.UpdateLogic(elapsedTime);
            Windows.ForEach(w => w.UpdateLogic(elapsedTime));
        }
